...
export const XOutput: React.FC<XOutputProps> = ({
  ...
  const strokeDashoffset = strokeDasharray - ((xRefined.engagementScore * 10) / 100) * strokeDasharray;

  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      className="bg-gradient-to-br from-gray-50 to-slate-50/60 border border-gray-900/10 rounded-[24px] p-6 flex flex-col gap-6"
    >
      {/* Header */}
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={isRefining}
          className="flex items-center gap-1.5 px-3 py-1.5 text-gray-900 hover:bg-gray-200/50 rounded-full text-xs font-semibold transition-all disabled:opacity-50"
        >
          {isRefining ? (
