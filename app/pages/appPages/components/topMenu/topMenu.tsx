...
import { ChevronDown, Coins, Bell, Sparkles, LayoutGrid, Plus, Check, Trash2, Loader2 } from "lucide-react";
...
const TopMenu: React.FC<TopMenuProps> = ({
  ...
  const handleSwitchAccount = async (accountId: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setIsDropdownOpen(false);
        // Navigate to same page under the new accountId
        // Handles both new paths (/{id}/page) and old paths (/pages/appPages/{id}/page)
  ...
  const activeAccount = accounts.find(a => a._id === activeAccountId) || accounts[0];
        ...
        <div className="relative z-50" ref={dropdownRef}>
          ...
          >
            ...
            <ChevronDown
              size={13}
              strokeWidth={3}
              className={`text-slate-300 group-hover:text-[#0052FF] transition-all ml-1 ${isDropdownOpen ? 'rotate-180' : ''}`}
            />
          </button>
          ...
          {isDropdownOpen && (
            <div className="absolute top-full right-0 mt-2 z-50 min-w-[280px] bg-white rounded-[24px] shadow-[0_20px_60px_rgba(0,0,0,0.12)] border border-slate-100 overflow-hidden flex flex-col">
              ...
              <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <span className="text-xs font-black text-slate-400 uppercase tracking-wider">Switch Account</span>
                <button 
              ...
              {/* Add Account Form */}
              {isAddingAccount && (
                <form onSubmit={handleAddAccount} className="p-3 border-b border-slate-100 bg-[#0052FF]/5">
              ...
              <div className="max-h-[300px] overflow-y-auto p-2 flex flex-col gap-1">
                {accounts.map((account) => {
                  ...
                  return (
                    <div 
                      key={account._id}
                      onClick={() => !isActive && handleSwitchAccount(account._id)}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`text-sm font-bold ${isActive ? 'text-[#0052FF]' : 'text-[#1A1D23]'}`}>
                          {account.name}
                        </span>
                      </div>
