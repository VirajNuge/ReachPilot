...
export default function PostSeedPanel({ postSeed, onChange, onReset }: PostSeedPanelProps) {
  ...
  const templateCoverage = postSeed.platforms.map((platform) => ({
    platform,
    templateId: postSeed.platformTemplateIds?.[platform] || postSeed.selectedTemplateId || "",
  }));
  ...
  return (
    <div className="space-y-1">
      ...
      <Section icon={Target} title="Primary Inputs">
        <div className="mb-3">
          ...
          <textarea
            ...
            placeholder="Write the detailed core message you want this post to communicate..."
          />
        </div>

        <MultiPill<PostPlatform>
          label="Platforms"
          selected={postSeed.platforms}
        ...
        <div className="mb-3">
          ...
          <textarea
            value={postSeed.imageConcept ?? ""}
            onChange={(e) => update({ imageConcept: e.target.value })}
            rows={4}
            className="w-full px-3 py-2 rounded-xl border border-slate-200 text-[12px] font-medium text-slate-700 focus:border-[#0052FF] focus:ring-1 focus:ring-[#0052FF]/20 outline-none resize-none"
            placeholder="Describe the single-post visual concept, composition, focal subject, and style cues..."
          />
        </div>

        <MultiPill<VisualStyle>
          label="Visual Styles"
          selected={postSeed.visualStyles}
          onChange={(v) => update({ visualStyles: v })}
          options={Object.entries(VISUAL_STYLE_LABELS).map(([k, v]) => ({ value: k as VisualStyle, label: v.label }))}
        />

        <div className="mb-3">
          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Color Palette</label>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {postSeed.brandAssets.colorPalette.map((color, index) => (
              <label key={`${color}-${index}`} className="flex items-center gap-2 p-2 rounded-lg border border-slate-200 bg-white">
                <span className="w-6 h-6 rounded-md border border-slate-200 shrink-0" style={{ backgroundColor: color }} />
                <input
                  value={color}
                  onChange={(e) => {
                    const nextPalette = [...postSeed.brandAssets.colorPalette];
                    nextPalette[index] = e.target.value;
                    updateAssets({ colorPalette: nextPalette });
                  }}
                  className="w-full text-[11px] font-bold text-slate-600 outline-none"
                />
              </label>
            ))}
          </div>
      ...
      <button type="button" onClick={() => setAdvancedOpen(!advancedOpen)}
        ...
        <span className="flex items-center gap-2"><Settings2 size={12} /> {advancedOpen ? "Hide Extra Inputs" : "Show Extra Inputs"}</span>
      ...
      {advancedOpen && (
        <div className="pl-1 space-y-1">
          ...
          <Section icon={Palette} title="Creative Base">
            ...
            <div className="mb-3">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Brand Controls</label>
          ...
          {templateCoverage.length > 0 && (
            <Section icon={Layers} title="Template Coverage">
              <div className="space-y-2">
                {templateCoverage.map((entry) => (
                  <div key={entry.platform} className="p-2.5 bg-slate-50 rounded-lg border border-slate-100 flex items-center justify-between gap-3">
                    <span className="text-[11px] font-bold text-slate-600 uppercase tracking-wider">{entry.platform}</span>
                    <span className="text-[11px] font-bold text-[#0052FF] bg-[#0052FF]/10 border border-[#0052FF]/15 px-2 py-0.5 rounded-md">
                      {entry.templateId || "Missing"}
                    </span>
                  </div>
                ))}
              </div>
